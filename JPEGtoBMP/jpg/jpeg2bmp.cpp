...
int LoadJpegFile (char *JpegFileName)
{
	...
	//to the file beginning position
...
{  
    ...
    if(pipe==NULL)   
    showerror(FUNC_FILE_ERROR);    //file not found
    else   {   
    fread(buffer,1,16*20,pipe);    
    }   
	
//find all the jpeg file	
while(i>=0&&i<16*20&&(buffer[i]!=(char)-2)){
		int j=0;
		...
		i+=2;
		if(j>=4){
			if(s[j]=='g'&&s[j-1]=='p'&&s[j-2]=='j'&&s[j-3]=='.'){
